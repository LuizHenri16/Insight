...
export const InvestimentosModal = () => {
    ...
    const rows = useMemo(() => {
        return (investimentos || []).map((investimento) => (
            <tr key={investimento.id_investimento} className="border-b border-slate-200 dark:border-zinc-700 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors">
    ...
    return (
        <div className="w-full flex flex-col gap-4">
            <div className="w-full flex gap-2 justify-between items-center">
                <h1 className="text-lg font-bold dark:text-zinc-100">Investimentos</h1>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-slate-200 dark:border-zinc-700">
                            ...
                            <th className="py-3 px-4 font-medium dark:text-zinc-200">Investimento</th>
                        ...
                        {isLoading ? (
                            <tr>
                                <td colSpan={3} className="py-10">
                                    <div className="flex flex-col items-center justify-center animate-pulse gap-2 text-slate-600 dark:text-zinc-300">
                            ...
                            investimentos?.length === 0 ? (
                                <tr>
                                    <td colSpan={3} className="py-3 px-4 text-center text-slate-600 dark:text-zinc-300">
                                        <Image src="/assets/images/no-data-image.svg" alt="Vazio" width={138} height={138} className="mx-auto" />
                                        <h3 className="mt-2 font-semibold text-slate-600 dark:text-zinc-300">Nenhum investimento encontrado</h3>
                                    </td>
                                </tr>
                            ) : (
                                rows
